...
## Purpose
LibreSpeed is a self-hosted network speed test server. This deployment runs it in standalone mode, exposing a web-based speed test interface on port 20011.

## Key Files
| File | Description |
|------|-------------|
| `docker-compose.yaml` | Docker Compose configuration for speedtest |
...
### External
- `ghcr.io/librespeed/speedtest:latest` — self-hosted network speed test server
